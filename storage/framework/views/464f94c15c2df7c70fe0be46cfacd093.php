<div class="view-section fade-in space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $summaryRows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="bg-slate-800 border border-slate-700 rounded-xl p-4 shadow-lg hover:border-slate-600 transition-colors">
            <div class="flex items-start justify-between mb-3">
                <div class="min-w-0">
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider truncate"><?php echo e($row->name); ?></p>
                    <p class="text-xs font-mono text-slate-500 mt-0.5"><?php echo e($row->ip_address); ?></p>
                </div>
                <span class="text-xs px-2 py-0.5 rounded-full font-bold flex-shrink-0 ml-2
                    <?php echo e($row->status === 'online' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
            ...
            <div class="grid grid-cols-3 gap-2 text-center">
                <div>
                    <div class="text-lg font-bold text-white"><?php echo e(number_format($row->avg_latency ?? 0, 0)); ?><span class="text-xs text-slate-500">ms</span></div>
                    <div class="text-[10px] text-slate-500 uppercase">Avg Lat.</div>
                </div>
                <div class="border-x border-slate-700">
                    <div class="text-lg font-bold <?php echo e(($row->avg_loss ?? 0) > 0 ? 'text-red-400' : 'text-emerald-400'); ?>"><?php echo e(number_format($row->avg_loss ?? 0, 1)); ?><span class="text-xs text-slate-500">%</span></div>
                    <div class="text-[10px] text-slate-500 uppercase">Loss</div>
                </div>
                <div>
                    <div class="text-lg font-bold text-slate-300"><?php echo e($row->check_count); ?></div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-700/50 text-slate-300">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $metrics; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-slate-700/20 transition-colors">
                        <td class="px-5 py-3 font-mono text-xs text-slate-400"><?php echo e($m->created_at->format('M d, H:i:s')); ?></td>
                        <td class="px-5 py-3 font-semibold text-white"><?php echo e($m->monitor?->name ?? '—'); ?></td>
                        <td class="px-5 py-3 font-mono text-slate-300"><?php echo e($m->monitor?->ip_address ?? '—'); ?></td>
                        <td class="px-5 py-3 font-mono">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($m->latency > 0): ?>
                                <span class="<?php echo e($m->latency > 200 ? 'text-red-400' : ($m->latency > 100 ? 'text-amber-400' : 'text-emerald-400')); ?>">
                                    <?php echo e(number_format($m->latency, 1)); ?>ms
                                </span>
                            <?php else: ?>
                                <span class="text-red-500 font-bold">TIMEOUT</span>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </td>
                        <td class="px-5 py-3 font-mono <?php echo e($m->packet_loss > 0 ? 'text-red-400' : 'text-slate-400'); ?>">
                            <?php echo e(number_format($m->packet_loss, 1)); ?>%
                        </td>
                        <td class="px-5 py-3">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($m->packet_loss >= 100): ?>
                                <span class="text-xs px-2 py-0.5 rounded-full bg-red-500/10 text-red-400 border border-red-500/20 font-bold">OFFLINE</span>
                            <?php elseif($m->packet_loss > 0): ?>
        ...
        <div id="export-modal"
            ...
            <div class="p-6 overflow-y-auto space-y-6 flex-1">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $monitors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $monitor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label wire:click="toggleIp(<?php echo e($monitor->id); ?>)"
                            class="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all
                                <?php echo e(in_array($monitor->id, $selectedIps)
                                    ? 'bg-emerald-500/10 border-emerald-500/40 text-white'
                                    : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500'); ?>">
                            <div class="h-5 w-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all
                                <?php echo e(in_array($monitor->id, $selectedIps) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-600'); ?>">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($monitor->id, $selectedIps)): ?>
                                <i class="ph-bold ph-check text-[10px] text-white"></i>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
                            <div class="min-w-0 flex-1">
                                <div class="font-semibold text-sm truncate"><?php echo e($monitor->name); ?></div>
                                <div class="font-mono text-xs text-slate-500"><?php echo e($monitor->ip_address); ?></div>
                            </div>
                            <span class="text-[10px] px-1.5 py-0.5 rounded font-bold flex-shrink-0
                                <?php echo e($monitor->status === 'online' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'); ?>">
                                <?php echo e(strtoupper($monitor->status)); ?>

                            </span>
                        </label>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(empty($selectedIps)): ?>
                    <p class="text-xs text-red-400 mt-2 flex items-center gap-1"><i class="ph ph-warning-circle"></i> Select at least one monitor.</p>
                    <?php else: ?>
                    <p class="text-xs text-slate-500 mt-2"><?php echo e(count($selectedIps)); ?> monitor(s) selected</p>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <select wire:model="groupBy"
                                class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500 outline-none <?php echo e($reportType === 'summary' ? 'opacity-40 cursor-not-allowed' : ''); ?>"
                                <?php echo e($reportType === 'summary' ? 'disabled' : ''); ?>>
                ...
                <div>
                    ...
                    <div class="flex gap-3">
                        <label class="flex-1 flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all
                            <?php echo e($exportFormat === 'csv' ? 'bg-emerald-500/10 border-emerald-500/40' : 'bg-slate-800 border-slate-700 hover:border-slate-500'); ?>">
                        ...
                        <label class="flex-1 flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all opacity-50 cursor-not-allowed
                            <?php echo e($exportFormat === 'json' ? 'bg-blue-500/10 border-blue-500/40' : 'bg-slate-800 border-slate-700'); ?>">
                ...
                <div class="bg-slate-800/80 border border-slate-700 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div class="text-xs text-slate-400 leading-relaxed">
                        ...
                        Exporting <strong class="text-white"><?php echo e(count($selectedIps)); ?> monitor(s)</strong>
                        from <strong class="text-white"><?php echo e($dateFrom); ?></strong> to <strong class="text-white"><?php echo e($dateTo); ?></strong>
                        as <strong class="text-emerald-400"><?php echo e(strtoupper($exportFormat)); ?></strong>
                        — <strong class="text-white"><?php echo e(collect(['summary'=>'Summary','latency'=>'Latency','packet_loss'=>'Packet Loss','full'=>'Full Data'])[$reportType]); ?></strong>
                        ...
                        grouped by <strong class="text-white"><?php echo e($groupBy); ?></strong>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="p-5 border-t border-slate-700 flex items-center justify-between gap-3 flex-shrink-0">
                ...
                <button wire:click="exportCsv"
                    <?php if(empty($selectedIps)): echo 'disabled'; endif; ?>
                    ...
                    <span wire:loading.remove wire:target="exportCsv">Download <?php echo e(strtoupper($exportFormat)); ?></span>
...
<?php /**PATH D:\laravel project\Network-Monitoring\resources\views/livewire/reports.blade.php ENDPATH**/ ?>