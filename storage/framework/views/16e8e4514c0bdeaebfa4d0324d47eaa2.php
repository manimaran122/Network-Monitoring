<div wire:poll.5s class="view-section fade-in space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 text-xs text-slate-400 bg-slate-800 border border-slate-700 px-3 py-2 rounded-xl">
            ...
            Live · <?php echo e(now()->format('H:i:s')); ?>
...
[$__name, $__params] = $__split('ai-insight', []);

$key = null;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-1842107775-0', null);
...
?>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-slate-800 border border-slate-700 hover:border-slate-600 rounded-2xl p-5 shadow-lg relative overflow-hidden transition-colors group">
            ...
            <div class="relative">
                ...
                <h3 class="text-3xl font-black text-white mt-1"><?php echo e($total); ?></h3>
            </div>
        </div>
        <div class="bg-slate-800 border border-emerald-500/20 hover:border-emerald-500/40 rounded-2xl p-5 shadow-lg relative overflow-hidden transition-colors group">
            ...
            <div class="relative">
                ...
                <h3 class="text-3xl font-black text-emerald-400 mt-1"><?php echo e($online); ?></h3>
            </div>
        </div>
        <a href="<?php echo e(route('alerts')); ?>" class="bg-slate-800 border <?php echo e($critical > 0 ? 'border-red-500/30 hover:border-red-500/60' : 'border-slate-700 hover:border-slate-600'); ?> rounded-2xl p-5 shadow-lg relative overflow-hidden transition-all group block">
            <div class="absolute inset-0 bg-gradient-to-br from-red-500/5 to-transparent rounded-2xl"></div>
            <div class="relative">
                <div class="h-10 w-10 rounded-xl <?php echo e($critical > 0 ? 'bg-red-500/10 text-red-400' : 'bg-slate-700 text-slate-400'); ?> flex items-center justify-center mb-3">
                    <i class="ph-fill ph-warning-octagon text-xl <?php echo e($critical > 0 ? 'animate-pulse' : ''); ?>"></i>
                </div>
                <p class="text-xs <?php echo e($critical > 0 ? 'text-red-400' : 'text-slate-400'); ?> font-bold uppercase tracking-wider">Critical Down</p>
                <h3 class="text-3xl font-black <?php echo e($critical > 0 ? 'text-red-400' : 'text-white'); ?> mt-1"><?php echo e($critical); ?></h3>
            </div>
        </a>
        <div class="bg-slate-800 border border-amber-500/20 hover:border-amber-500/40 rounded-2xl p-5 shadow-lg relative overflow-hidden transition-colors group">
            ...
            <div class="relative">
                ...
                <h3 class="text-3xl font-black text-white mt-1"><?php echo e($avgLatency); ?><span class="text-base text-slate-500 font-medium ml-1">ms</span></h3>
            </div>
        </div>
    </div>
    ...
    <div x-data="{
            ...
            jitterChart: null,
            lossChart: null,
            chartData: <?php echo e(json_encode($chartData)); ?>,
            avgJitter: 0,
            avgLoss: 0,
            initCharts() {
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
                ...
                <div class="flex items-center gap-2 text-xs">
                    <span class="px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold">
                        <?php echo e($chartData['monitorCount']); ?> active monitor<?php echo e($chartData['monitorCount'] !== 1 ? 's' : ''); ?>

                    </span>
                    <span class="px-2.5 py-1 rounded-full bg-slate-700 text-slate-300 border border-slate-600 font-bold font-mono">
                        avg <?php echo e($avgLatency); ?> ms
                    </span>
                </div>
            </div>
    ...
    <div class="bg-slate-800 border border-slate-700 rounded-2xl shadow-xl overflow-hidden">
        <div class="p-4 border-b border-slate-700 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('monitors')); ?>" wire:navigate
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-700/50">
                    ...
                    <tr class="hover:bg-slate-700/20 transition-colors <?php echo e($monitor->status === 'offline' ? 'border-l-2 border-l-red-500 bg-red-500/5' : ''); ?>">
                        <td class="px-5 py-3.5">
                            ...
                            <?php else: ?>
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20">
                                    <span class="h-1.5 w-1.5 rounded-full bg-amber-400"></span><?php echo e(strtoupper($monitor->status)); ?>

                                </span>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </td>
                        <td class="px-5 py-3.5 font-semibold text-white"><?php echo e($monitor->name); ?></td>
                        <td class="px-5 py-3.5 font-mono text-slate-400 text-xs"><?php echo e($monitor->ip_address); ?></td>
                        <td class="px-5 py-3.5 font-mono font-bold
                            <?php echo e($monitor->status === 'offline' ? 'text-red-400' : ($monitor->latency > 200 ? 'text-amber-400' : 'text-emerald-400')); ?>">
                            <?php echo e($monitor->status === 'offline' ? 'TIMEOUT' : number_format($monitor->latency) . ' ms'); ?>

                        </td>
                        <td class="px-5 py-3.5">
                            <?php $loss = $monitor->packet_loss ?? 0; ?>
                            <span class="font-mono font-bold <?php echo e($loss > 0 ? 'text-red-400' : 'text-slate-400'); ?>">
                                <?php echo e($loss); ?>%
                            </span>
                        </td>
                        <td class="px-5 py-3.5 text-slate-500 text-xs">
                            <?php echo e($monitor->last_check?->diffForHumans() ?? '—'); ?>
...
<?php /**PATH D:\laravel project\Network-Monitoring\resources\views/livewire/dashboard.blade.php ENDPATH**/ ?>